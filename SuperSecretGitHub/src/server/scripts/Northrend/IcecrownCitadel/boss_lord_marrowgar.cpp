                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BONE_SPIKE_GRAVEYARD:
                            ...
                            events.ScheduleEvent(EVENT_BONE_SPIKE_GRAVEYARD, urand(16000, 20000), EVENT_GROUP_SPECIAL);
                            break;
    ...
    public:
        ...
        struct npc_coldflameAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                {
                    Position newPos;
                    me->GetNearPosition(newPos, 1.5f, 0.0f);
...
class spell_marrowgar_bone_storm : public SpellScriptLoader
{
    public:
        ...
        class spell_marrowgar_bone_storm_SpellScript : public SpellScript
        {
            ...
            {
				SpellEntry const* spellInfo = sSpellMgr->GetSpellForDifficultyFromSpell(GetSpellInfo(), GetCaster());
				if(!spellInfo)
					return;
				
				// 6000 (Norm10), 12000 (Heroic 10), 12000 (Norm 25), 14000 (Heroic 25)
				// Base * 4 / Distance
				// 4 = 10/2.5
                float distance = 4.0f / GetHitUnit()->GetExactDist2d(GetCaster());
				if(distance < 4.0f)
					distance = 1.0f;
				SetHitDamage(int32(spellInfo->EffectBasePoints[0] * distance));
            }
...
class spell_marrowgar_bone_slice : public SpellScriptLoader
{
    public:
        ...
        class spell_marrowgar_bone_slice_SpellScript : public SpellScript
        {
            ...
            {
				std::list<HostileReference*>::const_iterator i = GetOriginalCaster()->getThreatManager().getThreatList().begin();
				int count = 0;
				for (; i != GetOriginalCaster()->getThreatManager().getThreatList().end(); ++i)
				{
					if(count == 2)
						break;

					Unit *pTarget = (*i)->getTarget();
					if (pTarget->isAlive() && GetOriginalCaster()->IsWithinMeleeRange(pTarget))
					{
						unitList.push_back(pTarget);
						++count;
					}
				}
            }
            ...
            void Register()
            {
                OnUnitTargetSelect += SpellUnitTargetFn(spell_marrowgar_bone_slice_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_AREA_ENTRY_DST);
            }
        };
