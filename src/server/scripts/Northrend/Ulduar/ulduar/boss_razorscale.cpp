...
{
    struct NPC_expedition_commander_ulduar
        : CreatureScript
    {
        NPC_expedition_commander_ulduar()
            : CreatureScript("npc_expedition_commander_ulduar")
        {
        }

        bool OnGossipHello(Player *player, Creature *creature)
        {
            return Expedition_commander_ulduar(std::move(player), std::move(creature));
        }

        bool OnGossipSelect
            (Player *player, Creature *creature, uint32 sender, uint32 action)
        {
            return GossipSelect_commander_ulduar(std::move(player), std::move(creature),
                    std::move(sender), std::move(action));
        }

        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_commander_ulduar(std::move(creature));
        }
    };
        ...
        : CreatureScript
    {
        ...
        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_boss_razorscale(std::move(creature));
        }
    };
    new Boss_razorscale;
    
    struct Mob_devouring_flame
        : CreatureScript
    {
        Mob_devouring_flame()
            : CreatureScript("mob_devouring_flame")
        {
        }

        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_mob_devouring_flame(std::move(creature));
        }
    };
    new Mob_devouring_flame;
    
    struct Mob_darkrune_watcher
        : CreatureScript
    {
        Mob_darkrune_watcher()
            : CreatureScript("mob_darkrune_watcher")
        {
        }

        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_mob_darkrune_watcher(std::move(creature));
        }
    };
    new Mob_darkrune_watcher;
    
    struct Mob_darkrune_guardian
        : CreatureScript
    {
        Mob_darkrune_guardian()
            : CreatureScript("mob_darkrune_guardian")
        {
        }

        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_mob_darkrune_guardian(std::move(creature));
        }
    };
    new Mob_darkrune_guardian;
    
    struct Mob_darkrune_sentinel
        : CreatureScript
    {
        Mob_darkrune_sentinel()
            : CreatureScript("mob_darkrune_sentinel")
        {
        }

        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_mob_darkrune_sentinel(std::move(creature));
        }
    };
    new Mob_darkrune_sentinel;
    
    struct Mole_machine_trigger
        : CreatureScript
    {
        Mole_machine_trigger()
            : CreatureScript("mole_machine_trigger")
        {
        }

        CreatureAI *GetAI(Creature *creature) const
        {
            return GetAI_mole_machine_trigger(std::move(creature));
        }
    };
