...
bool ReputationMgr::SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental)
{
    ...
    if (itr != _factions.end())
    {
        ...
        if (incremental)
        {
          ...
          standing = int32(floor((float)standing * (_player->GetPlayerRate(PLAYER_RATE_REPUTATION)?_player->GetPlayerRate(PLAYER_RATE_REPUTATION) : sWorld->getRate(RATE_REPUTATION_GAIN)) + 0.5f));
