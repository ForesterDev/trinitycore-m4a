...
bool GlobalCooldownMgr::HasGlobalCooldown(SpellInfo const* spellInfo, int offset) const
{
    GlobalCooldownList::const_iterator itr = m_GlobalCooldowns.find(spellInfo->StartRecoveryCategory);
    return itr != m_GlobalCooldowns.end() && itr->second.duration && getMSTimeDiff(itr->second.cast_time, getMSTime()) + offset < itr->second.duration;
}
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    else
    {
        ...
        dodge_chance += 100 * GetTotalAuraModifier(SPELL_AURA_MOD_ENEMY_DODGE);
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (canDodge)
    {
        ...
        dodgeChance += 100 * GetTotalAuraModifier(SPELL_AURA_MOD_ENEMY_DODGE);
        // Reduce dodge chance by attacker expertise rating
        if (GetTypeId() == TYPEID_PLAYER)
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (TakenTotalCasterMod)
    {
        ...
        {
            if (TakenTotalMod < 1)
                tmpDamage = (((CalculatePct(float(pdamage), TakenTotalCasterMod) + TakenTotal) * TakenTotalMod) + CalculatePct(float(pdamage), TakenTotalCasterMod));
            else
                tmpDamage = (((CalculatePct(float(pdamage), TakenTotalCasterMod) + TakenTotal) + CalculatePct(float(pdamage), TakenTotalCasterMod)) * TakenTotalMod);
        }
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (TakenTotalCasterMod)
    {
        ...
        {
            if (TakenTotalMod < 1)
                tmpDamage = (((CalculatePct(float(pdamage), TakenTotalCasterMod) + TakenFlatBenefit) * TakenTotalMod) + CalculatePct(float(pdamage), TakenTotalCasterMod));
            else
                tmpDamage = (((CalculatePct(float(pdamage), TakenTotalCasterMod) + TakenFlatBenefit) + CalculatePct(float(pdamage), TakenTotalCasterMod)) * TakenTotalMod);
        }
        else if (TakenTotalMod < 1)
...
void Unit::ModSpellCastTime(SpellInfo const* spellProto, int32 & castTime, Spell* spell, bool round_toward_infinity)
{
    ...
    {
        auto tmp = float(castTime) * GetFloatValue(UNIT_MOD_CAST_SPEED);
        castTime = round_toward_infinity ? int32(std::ceil(tmp)) : int32(tmp);
    }
...
int Unit::spell_start_recovery_offset() const
{
    return IsControlledByPlayer() ? 400 : 0;
}
