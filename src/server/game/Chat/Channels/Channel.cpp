...
Channel::Channel(std::string const& name, uint32 channelId, uint32 team, bool custom):
    _announce(true),
    _ownership(true),
    _IsSaved(false),
    _flags(0),
    ...
    _password("")
{
    ...
    {
        _announce = false;
        _flags |= CHANNEL_FLAG_NOT_LFG | CHANNEL_FLAG_GENERAL;
        _IsSaved = false;
        is_global = true;
    }
...
void Channel::JoinChannel(Player* player, std::string const& pass)
{
    ...
    if (player_level_ok(*player))
        ;
    else
    {
        ChatHandler(player->GetSession()).PSendSysMessage
            ("You do not meet the level requirements of %s.", _name.c_str());
        WorldPacket data;
        MakeBanned(&data);
        SendToOne(&data, guid);
        return;
