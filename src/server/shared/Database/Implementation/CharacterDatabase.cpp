...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_DEL_EXPIRED_BAN, "UPDATE character_banned SET active = 0 WHERE guid=? and bandate=?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHARACTER_BAN, "UPDATE character_banned SET active = 0 WHERE guid = ? AND active = 1", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_BANINFO, "SELECT cast(FROM_UNIXTIME(bandate) as char), unbandate-bandate, active, unbandate, banreason, bannedby FROM character_banned WHERE guid = ? ORDER BY bandate ASC", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_ENUM, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.playerBytes, c.playerBytes2, c.level, c.zone, c.map, c.position_x, c.position_y, c.position_z, "
                     "gm.guildid, c.playerFlags, c.at_login, cp.entry, cp.modelid, cp.level, c.equipmentCache, cb.bandate, cb.unbandate <= UNIX_TIMESTAMP() AND cb.unbandate <> cb.bandate "
    ...
    PrepareStatement(CHAR_SEL_ENUM_DECLINED_NAME, "SELECT c.guid, c.name, c.race, c.class, c.gender, c.playerBytes, c.playerBytes2, c.level, c.zone, c.map, "
                     ...
                     "cb.bandate, cb.unbandate <= UNIX_TIMESTAMP() AND cb.unbandate <> cb.bandate, cd.genitive FROM characters AS c LEFT JOIN character_pet AS cp ON c.guid = cp.owner AND cp.slot = ? "
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_BANNED, "SELECT guid FROM character_banned WHERE guid = ? AND active = 1 and (unbandate=bandate or unbandate>UNIX_TIMESTAMP())", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_INVALID_ACHIEV_PROGRESS_CRITERIA, "DELETE FROM character_achievement_progress WHERE guid = ? and criteria = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PINFO_BANS, "SELECT unbandate, bandate = unbandate, bannedby, banreason FROM character_banned WHERE guid = ? AND active = 1 and (unbandate=bandate or unbandate>UNIX_TIMESTAMP()) ORDER BY bandate ASC LIMIT 1", CONNECTION_SYNCH);
    ...
    // Calendar
    PrepareStatement(CHAR_REP_CALENDAR_EVENT, "REPLACE INTO calendar_events (id, creator, title, description, type, dungeon, event_time, flags, lockout_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CALENDAR_EVENT, "DELETE FROM calendar_events WHERE id = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CALENDAR_INVITE, "REPLACE INTO calendar_invites (id, event, invitee, sender, status, response_time, rank, type, text) VALUES (?, ?, ?, ?, ?, ?, ?, ?+1, ?)", CONNECTION_ASYNC);
